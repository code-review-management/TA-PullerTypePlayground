...
export function SuggestionDiffEditor(props: SuggestionDiffEditorProps) {
  ...
  const [editorOriginal, setEditorOriginal] = useState(() => buildFullCode(beforeCode, originalCode, afterCode));
  const [editorModified, setEditorModified] = useState(() => buildFullCode(beforeCode, modifiedCode, afterCode));

  const [prevBoundaries, setPrevBoundaries] = useState({ before: beforeCode, after: afterCode });

  if (beforeCode !== prevBoundaries.before || afterCode !== prevBoundaries.after) {
