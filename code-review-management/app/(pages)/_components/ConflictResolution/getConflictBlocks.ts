...
import { MergeFileOutput } from "@/lib/merge-conflict-finder/merge-github.types";
 ...
 */
export default function getConflictBlocks(mergeFileOutput: MergeFileOutput): Map<number, ConflictBlock> {
    const conflictBlocks= new Map<number, ConflictBlock>();

    let currentStart = -1
    let currentDivider = -1

    const value = mergeFileOutput.contents
