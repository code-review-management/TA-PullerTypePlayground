/*
/api/v2/{owner}/{repo}/pulls/{pull_number}/reviews

*NOT TO BE POLLED*
*/

import { getCookieName } from "@/app/api/_utils/cookie-utils";
import { Review, ReviewSchema } from "@/types/github.types";
...
export async function GET(req: Request, context: RouteContext) {
  ...
  try {
    ...
    // Filter response
    const filteredResponse: Review[] = data.data.map((item) =>
      ReviewSchema.parse(item),
    );

    const linkHeaders = parse(data.headers.link);

    const wrappedResponse: ReviewV2 = {
