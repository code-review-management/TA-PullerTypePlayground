/*
/api/v2/{owner}/{repo}/collaborators

*NOT TO BE POLLED*
*/

import { getCookieName } from "@/app/api/_utils/cookie-utils";
import { User, UserSchema } from "@/types/github.types";
import { UserV2 } from "@/types/github.types.wrapper";
...
export async function GET(req: Request, context: RouteContext) {
  ...
  try {
    ...
    // Filter response
    const filteredResponse: User[] = data.data.map((item) =>
      UserSchema.parse(item),
    );

    const linkHeaders = parse(data.headers.link);

    const wrappedResponse: UserV2 = {
